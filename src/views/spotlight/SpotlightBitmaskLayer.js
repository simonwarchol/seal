...
bool isOutline(sampler2D dataTex, vec2 coord) {
...
vec4 sampleAndGetColor(sampler2D dataTex, vec2 coord, bool isOn) {
    ...
    // Check if sampled color is 200,200,200 (selection color)
    if (sampledColor.r == 0.7843137254901961 && 
        sampledColor.g == 0.7843137254901961 && 
        sampledColor.b == 0.7843137254901961) {
        // For selected cells
        if (spotlightSelection) {
          sampledColor = vec4(0.00000001, 0.0, 0.0, 1.0); // Black outline
        } else {
          sampledColor = vec4(0.0, 0.0, 0.0, 1.0); // Black outline
        }
            } else if (sampledColor.r == 0. && sampledColor.g == 0. && sampledColor.b == 0.) {
        // Background handling
        if (selectedBackground) {
            sampledColor = vec4(0.00000001, 0.0, 0.0, 1.0);
        } else {
            sampledColor = vec4(0.);
        }
    } else {
        // Non-selected cells should be transparent
        sampledColor = vec4(0.0);
        if (isOutline(dataTex, coord) && outlineSelection) {
          sampledColor = vec4(1.0); // Red outline
        }
    }
